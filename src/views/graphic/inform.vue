...
export default {
  ...
  methods: {
    ...
    // 修改按钮
    handleUpdate(row) {
      this.$request.findByIdNews(row.id).then((res) => {
        console.log(res);
      });
      this.$router.push({
        path: "/detailgars",
    ...
    getList() {
      this.loading = true;
      this.$request.findByPageNews(this.params).then((res) => {
    ...
    handleDelete(row) {
      ...
      this.$confirm("是否确认删除该记录?", "警告", {
        ...
        .then(function () {
          that.$request.deleteNews(id).then((res) => {
            if (res.data.status == 200) {
              that.getList();
