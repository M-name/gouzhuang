<template>
  <div class="contant">
    ...
    <div class="left">
      ...
      <el-button
        v-if="tenatPeople.isOrNo"
        class="people-submit"
        type="primary"
        @click="edit"
        >修改</el-button
      >
      <el-button v-else class="people-submit" type="primary" @click="submitForm"
        >保存</el-button
      >
...
export default {
  data() {
    ...
    return {
      ...
      peopleRules: {
        ...
        faceAuthStatus: [
          { required: true, message: "人脸对比状态未选择", trigger: "blur" },
        ],
        countryId: [
  ...
  mounted() {
    if (this.$route.params.id) {
      localStorage.setItem("tenatPeople", JSON.stringify(this.$route.params));
      this.tenatPeople = this.$route.params;
    } else {
      this.tenatPeople = JSON.parse(localStorage.getItem("tenatPeople"));
    }
    //请求住户信息
    this.$request.detailsFindRenter(this.tenatPeople.id).then((res) => {
      if (res.data.status === 200) {
        this.faceFileImg = [];
    ...
    submitForm() {
      ...
      this.$refs["form"].validate((valid) => {
        if (valid) {
          ...
          this.$request.userCreate(this.form).then((res) => {
            if (res.data.status === 200) {
              this.faceFileImg = [];
              this.faceUploadImgList = [];
              this.infoFileImg = [];
    ...
    liveSubmitForm() {
      ...
      this.$refs["placeForm"].validate((valid) => {
        if (valid) {
          ...
          this.$request.buildingCreateRenter(this.placeForm).then((res) => {
            if (res.data.status === 200) {
