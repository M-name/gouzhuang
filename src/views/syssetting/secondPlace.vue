...
export default {
  ...
  mounted() {
    if (this.$route.params.code) {
      localStorage.setItem("secondCode", this.$route.params.code);
      this.params.parentCode = this.$route.params.code;
      this.code = this.$route.params.code;
    } else {
      this.params.parentCode = localStorage.getItem("secondCode");
      this.code = localStorage.getItem("secondCode");
    }
  ...
  methods: {
      ...
      this.$refs["form"].validate((valid) => {
        if (valid) {
          if (this.form.createTime) {
            ...
            this.$request.createCity(this.form).then((res) => {
              if (res.data.status === 200) {
    ...
    getList() {
      this.loading = true;
      this.$request.allCity(this.params).then((res) => {
        if (res.data.status === 200) {
          this.loading = false;
