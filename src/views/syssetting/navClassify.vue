...
export default {
  ...
  data() {
    return {
      ...
      //表格列
      column: [
        { prop: "dictName", label: "一级字典名称", widht: "180" },
        { prop: "dictCode", label: "一级字典编号" },
  ...
  methods: {
      ...
      this.$refs["form"].validate((valid) => {
        if (valid) {
          if (this.form.createTime) {
            ...
            this.$request.createFirst(this.form).then((res) => {
              if (res.data.status === 200) {
    ...
    getList() {
      this.loading = true;
      this.$request.allFirst(this.params).then((res) => {
        if (res.data.status === 200) {
          this.loading = false;
    ...
    handleDelete(index, row) {
      ...
      this.$confirm("是否确认删除该字典值?", "警告", {
        ...
        .then(function () {
          that.$request.delFirst(row.id).then((res) => {
           if (res.data.status === 200) {
              that.getList();
