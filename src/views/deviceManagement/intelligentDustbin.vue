...
export default {
  ...
  data() {
    return {
      ...
      heartbeatColumn: [
        { prop: "batteryVoltage", label: "电池电压" },
        { prop: "batterytemperatur", label: "电池温度" },
        { prop: "binCount", label: "分桶总数" },
        { prop: "capacity", label: "当前容量" },
        { prop: "chargingCurrent", label: "充电电流" },
        { prop: "chargingVoltage", label: "充电电压" },
        { prop: "dailyCharge", label: "日充电量" },
        { prop: "dailyElectricity", label: "日用电量" },
        { prop: "deviceGpsLat", label: "设备经纬度" },
        { prop: "iccId", label: "设备SIM卡" },
        { prop: "mainboardTemperature", label: "主板温度" },
        { prop: "signalIntensity", label: "信号强度" },
        { prop: "sortSum", label: "分类桶数量" },
        { prop: "temperature", label: "当前温度" },
        { prop: "waterLine", label: "吃水线" },
        { prop: "weight", label: "当前重量" },
      ],
      // 字典数据
      dictList: [],
      // 表格参数
  ...
  created() {
    this.$request.deviceStatistics("0").then((res) => {
  ...
  methods: {
    ...
    heartbeatGetList() {
      ...
      this.$request.deviceFindWS(this.heartbeatParams).then((res) => {
        if (res.data.status == 200) {
