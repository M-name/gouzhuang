    ...
    >
      <el-form ref="form" :model="form" :rules="rules" label-width="100px">
        ...
        <el-form-item label="收费标准" prop="feeRate">
          <el-input v-model="form.feeRate" placeholder="请输入字典名称" />
        </el-form-item>
...
export default {
  ...
  mounted() {
    if (this.$route.params.code) {
      localStorage.setItem("expensedetail", JSON.stringify(this.$route.params));
      this.expensedetail = this.$route.params;
    } else {
      this.expensedetail = JSON.parse(localStorage.getItem("expensedetail"));
    }
    this.params.parentCode = this.expensedetail.code;
    //获取费用字典
    this.$request
      .feeFindEnum({ feeCode: this.expensedetail.code })
      .then((res) => {
        let role = [];
        let lists = res.data.data;
        for (var i = 0; i < lists.feeDictSecondEnum.length; i++) {
          let arr = {};
          arr.value = lists.feeDictSecondEnum[i].feeCode;
          arr.label = lists.feeDictSecondEnum[i].feeName;
          role.push(arr);
        }
  ...
  methods: {
    ...
    resetQuery() {
      this.params = {
        ...
        page: 1,
        pageSize: 20,
        parentCode: this.expensedetail.code,
      };
      this.getList();
    },
    // 表格单个禁用
      ...
      this.$refs["form"].validate((valid) => {
        if (valid) {
          if (this.form.createTime) {
            ...
            this.$request.feeCreateSecond(this.form).then((res) => {
              if (res.data.status === 200) {
    ...
    //获取字典列表
    getList() {
      this.$request.feeAllSecond(this.params).then((res) => {
        if (res.data.status === 200) {
